...
export function HardSkillsCard({ skills, setSkills }: { skills: { name: string, value: number, wanted?: number }[], setSkills: Function }) {
  ...
  if (typeof window !== "undefined") {
    useEffect(() => {
      if (!localStorage?.getItem('hardSkills')) localStorage?.setItem('hardSkills', JSON.stringify(skills || []))
    }, [skills])
  }
  return (
    <motion.article
    ...
    >
      <SkillsChart key={skills?.length} title="HardSkills" dataSkills={skills} />
      <div className="px-16">
        <BaseDialog
          buttonText={
